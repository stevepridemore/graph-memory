...
Capture aliases and identifying details inline in the edge `evidence` string when the source mentioned them. Examples: "Alice Johnson (nickname AJ, initials ALJ)", "Postgres 16.2 (the version that introduced logical replication improvements)". This makes future fuzzy matching much better.
...
```
Would create:
  - Alice Johnson (Person) — Senior Engineer at Acme Corp
  - Q4 Migration Timeline (Decision) — confidence 0.7
Would link:
  - Alice Johnson -[STAKEHOLDER_IN, weight 0.6]-> Q4 Migration
  - ...
Would boost (already in graph):
  - Bob Lee: confirmed mentioned in Q4 Migration context — +0.05
```
